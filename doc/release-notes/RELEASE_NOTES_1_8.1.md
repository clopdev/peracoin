...
Peracoin Core 1.8.1 is primarily a bugfix release, bringing Peracoin Core in
line with Bitcoin 0.9.3. Peracoin Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by Peracoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Peracoin Core clients
* Use HTTPS with perachain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/peracoin/peracoin/issues/134).
...
This only affects values significantly below 1 Pera, however merchants may wish to round to
the nearest Pera on all transactions for now.
