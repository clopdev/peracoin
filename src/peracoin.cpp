...
// Peracoin: Normally minimum difficulty blocks can only occur in between
...
unsigned int CalculatePeracoinNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
...
CAmount GetPeracoinMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    ...
    nMinFee += GetPeracoinDustFee(tx.vout, ::minRelayTxFee);
