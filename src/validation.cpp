...
#if defined(NDEBUG)
# error "Peracoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Peracoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetPeracoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Peracoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("peracoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Peracoin: BIP30 has been active since inception
    ...
    // Peracoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetPeracoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Peracoin: Disallow AuxPow blocks before it is activated.
    ...
    // Peracoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Peracoin: We probably want to disable this
    int nLockTimeFlags = 0;
